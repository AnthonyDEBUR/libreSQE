##### Export des résultats 2025 en QUESU v3 #####
...
Analyses <- Analyses[!(Analyses$CdRqAna %in% c("10") &
                         is.na(Analyses$RsAna)),]
...
fichier_a_importer<-paste0(nomfichier, "_quesu3.0.xml")
# fichier_a_importer<-file.choose()

tmp <- func_importe_quesu(fichier_a_importer)
...
summary(Anal_rslt$CdPointEauxSurf%>%as.factor)
 summary(Anal_rslt$CdRdd%>%as.factor)

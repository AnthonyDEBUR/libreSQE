...
ss_func_extrat_from_xml <- function(xml_nodes,
                                    ...
                                    scheme = NULL,
                                    count = NULL) {
  if (!("xml_nodeset" %in% class(xml_nodes))) {
    stop("ss_func_extrat_from_xml : xml_nodes should be a validate xml_nodeset")
  }
  if (!(is.list(values) |
        is.null(values))) {
    stop("ss_func_extrat_from_xml : values should be a list")
  }
  if (!(is.list(scheme) |
        is.null(scheme))) {
    stop("ss_func_extrat_from_xml : scheme should be a list")
  }
  if (!(is.list(count) |
        is.null(count))) {
    stop("ss_func_extrat_from_xml : count should be a list")
  }
  if (all(is.null(values),
          is.null(scheme),
          is.null(count))) {
    stop(
      "ss_func_extrat_from_xml : scheme, count and values can\'t be all NULL at the same time"
    )
  }
  ...
  if (length(nm_scheme) == 0) {
    nm_scheme <- NULL
  } else{
    nm_scheme <- paste0("type_", nm_scheme)
  }
  nm_count <- sapply(count, paste, sep = "", collapse = "_")
  if (length(nm_count) == 0) {
    nm_count <- NULL
  } else{
    nm_count <- paste0("nb_", nm_count)
  }
  ...
  df_out <-
    data.frame(matrix(ncol = length(colonnes), nrow = longueur))
  ...
  extract_node <- function(xml_nodes, chemin) {
    path_node<-paste(chemin, collapse = "/")
    node <- xml2::xml_find_all(xml_nodes,path_node, flatten = FALSE)

    # Extraire le texte du noeud final
    output <-
      lapply(node, function(x)
        (paste(xml2::xml_text(x), collapse = "/"))) %>% unlist

    return(output)
  }
  # fct remplissage du data frame avec les noms de la liste
  ...
  df_out[, nm_values] <-
    lapply(seq_along(nm_values), function(x)
      remplissage_df(x, values)) %>% as.data.frame
  ...
  extract_node_scheme <- function(xml_nodes, chemin) {
    ...
    # Extraire l'attribut du noeud final
  ...
  compter_balises <- function(balises_a_lire, xml_nodes, base) {
    ...
    res <- data.frame(matrix(
      0,
      nrow = length(xml_nodes),
      ncol = length(balises_a_lire)
    ))
    colnames(res) <- balises_a_lire


    for (i in seq_along(xml_nodes)){
                            node<-xml_nodes[[i]]
                            for (balise in balises_a_lire) {
                              res[i, balise] <-
                                length(xml2::xml_find_all(node, paste0("./", balise)))
                            }
                      }

...
bdc_quantitatif<-pool::dbGetQuery(conn,
...
bdc_quantitatif_station<-pool::dbGetQuery(conn,
                                  paste0("SELECT * FROM sqe.view_bdc_quantif_par_staq
                                      WHERE bco_id=",bdc_id,
                                         " AND pre_id IN (",
                                         paste(pre_id, collapse=","),");")
)
...
bdc_quantitatif<-bdc_quantitatif%>%subset(pru_datedebut<=date_mini & pru_datefin>=date_maxi)
...
if(nrow(bdc_quantitatif)!=nrow(bdc_quantitatif%>%dplyr::select(-pru_datedebut, -pru_datefin, -pru_valeur))){stop(paste0("Il existe des ambiguités sur les prix unitaire dans le programme / bon de commande ", bdc_quantitatif$bco_refcommande[1],". Vérifiez que le bon de commande ne couvre pas une période pendant laquelle les prix unitaires ont été révisés."))}
...
bdc_quantitatif$totalHT<-bdc_quantitatif$pru_valeur*bdc_quantitatif$bcq_nbprestacom
col_num_presta<-data.frame(col_num_presta=bdc_quantitatif$prs_idprestationdansbpu)
col_nom_presta<-data.frame(col_nom_presta=bdc_quantitatif$prs_label_prestation)
col_unite_presta<-data.frame(col_unite_presta=bdc_quantitatif$prm_unitedoeuvre)
...
id_nb_stations<-length(bdc_quantitatif_station[bdc_quantitatif_station$bcp_stm_cdstationmesureinterne!="sans_objet",]$bcp_stm_cdstationmesureinterne%>%unique())


bdc_quantitatif_station<-bdc_quantitatif_station%>%
  tidyr::pivot_wider(id_cols=c("bco_per_nom",
...
tryCatch(wb_trame<-openxlsx::loadWorkbook(bdc_type),
         error=function(e) e)

liste_sheet<-openxlsx::getSheetNames(bdc_type)
if(!"trame"%in%liste_sheet){stop(paste0("Fichier bon de commande type : ", bdc_type, " onglet trame (décrivant les champs à insérer) manquant."))}

liste_sheet<-liste_sheet[liste_sheet!="trame"]
if(length(liste_sheet)==0){stop("Aucun onglet à générer dans le fichier bon de commande type : ", bdc_type)}
...
for(i in 1:nrow(trame))
{
  if(substr(trame$Champs[i],1,2)=="id"){
    ...
    openxlsx::writeData(wb_trame,
                        sheet=trame$Onglet[i],
                        x=valeur,
                        xy=c(gsub("[[:digit:]]", "", trame$Emplacement[i]),
